...
return {
	...
	-- fancy top buffer manager, @see https://github.com/akinsho/bufferline.nvim
	{
		...
		event = "User FileOpened",
		keys = {
			{ "<left>", "<CMD> BufferLineCyclePrev <CR>", desc = "Goto left buffer" },
			{ "<right>", "<CMD> BufferLineCycleNext <CR>", desc = "Goto right buffer" },
			{ "<up>", "<CMD> BufferLineCloseOthers <CR>", desc = "Close other buffers" },
			{ "<down>", ":e#<CR>", desc = "Toggle to previous buffer" },
		},
		opts = {
			options = {
						...
						-- display attached lsp clients, formatters and linters
						{
							function()
								...
								if not buf_clients or #buf_clients == 0 then
									-- return "" -- icons.ui.Code -- no LSP attached
									return icons.ui.Code .. " " .. "N/A" -- no LSP attached
								end
